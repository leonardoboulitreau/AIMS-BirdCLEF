...
embedding_path = "embeddings.py"

# Output directory for analysis results
output_dir = "out_analyzer"
embedding_out_dir = "out_analyzer/embeddings"
...
for root, dirs, files in os.walk(audio_dir):
    for file in files:
        if file.endswith('.ogg'):
            ...
            # Run the analysis
            subprocess.run(['python', analyze_path, '--i', file_path, '--o', output_dir, '--slist', species_dir], check=True)

            # Generate embeddings
            embeddings_output_path = os.path.join(embedding_out_dir, os.path.splitext(file)[0] + '.embeddings.txt')
            subprocess.run(['python', embedding_path, '--i', file_path, '--o', embeddings_output_path], check=True)
