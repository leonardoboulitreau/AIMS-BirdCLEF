...
def get_loss(training_args, sample_distribution=None):
    if training_args['loss'] == 'balce':
        return BalancedCrossEntropyLoss(samples_per_class=sample_distribution)
    elif training_args['loss'] == 'ce':
...
def focal_loss(logits, labels, alpha=None, gamma=2):
    ...
    Returns:
      focal_loss: A float
      32 scalar representing normalized total loss.
    """
